...
## D21 — Phase 6 review gate resolved (2026-08-16)

Phase 5 is built and merged (PR #11). G5.1–G5.5 pass, seven of Codex's eight adversarial findings are fixed and the eighth is recorded as open (D20), and all four CI jobs are green. The Phase 6 spec is **Approved** with seven decisions.

Phase 6 is unusual: the backend is finished. Every one of the ten MVP screens already has an endpoint behind it with its role gate enforced server-side, so this phase adds **no new API surface** — which makes the decisions below almost entirely about what we take on as frontend dependencies and how we prove a browser journey.

1. **React Router + TanStack Query.** The screens that matter — run detail, approval inbox, ingestion status — are all "poll until this settles", and that is precisely what a query library does correctly and what hand-rolled `useEffect` gets subtly wrong (races on unmount, stale closures, no dedupe). Rejected zero-dependency routing: we would be hand-writing caching and polling for ten screens, which is more code we own and more code to review than two dependencies a reviewer already knows.
2. **Hand-written CSS with design tokens, no framework.** ~500 lines of stylesheet over tables, cards, forms and status badges. Tailwind was rejected for a portfolio piece specifically because its output is recognisable as Tailwind, and a component library was rejected as a heavy dependency that makes every screen look like every other app using it. This is the layer the demo recording is judged on, so it should look like ours.
3. **Polling, not SSE.** Run detail 2s until terminal, inbox 5s, documents 3s while processing, dashboard 15s. SSE would need a new authenticated streaming endpoint, per-tenant fan-out and a worker→API notification path — real backend work smuggled into a frontend phase, to shave seconds off waits that are architecturally minutes long (the durable pause is the whole point). Polling also degrades correctly when a connection drops.
4. **Playwright for the journey, Vitest for the pieces.** G6.1 drives the full ten-step definition of done through a real browser against the real stack, which is the only honest way to assert "the demo works". Component tests cover role gating and error states where a browser is overkill. A Vitest-only suite was rejected: asserting the MVP journey against a mocked `fetch` proves the mock, and closing exactly that gap is what every prior phase's gates exist for.
5. **The read-only agent config panel is in; user/role management is out.** CLAUDE.md explicitly blesses the former ("a read-only config page is sufficient") and `GET /api/config/agent` already serves it. The Administrator persona does mention managing users, but the ten-screen list omits it and **no endpoint exists** — building it means new backend write APIs inside a frontend phase. Roles stay seeded by `scripts/seed.py`; this is recorded as a known MVP limitation rather than quietly dropped.
6. **No charting dependency.** The draft spec's "one lightweight chart lib" is superseded — a dependency earning its place on one chart is a dependency that has not earned its place. *(Widened 2026-08-16: the dashboard now carries an area chart, a donut, ring gauges and sparklines, all hand-rolled in `components/charts.tsx`. The reasoning survives the extra scope — ~250 lines of SVG against a ~90 kB dependency, and they inherit the theme through CSS custom properties for free, where every charting library needs a theme adapter written twice. Nothing there computes statistics; it draws shapes from numbers the API already returned.)*

**Every series is real.** There is no time-series endpoint, so the activity chart buckets `/api/runs` client-side and the donut counts actual run outcomes. Nothing is padded, smoothed or invented to improve a curve, and the bucket grain follows the data — hours when everything is recent, days otherwise — because a fresh install bucketed over 30 days is 29 empty columns and one spike. A dashboard that draws a pleasing shape from nothing is the most expensive kind of lie in a product whose entire claim is that its numbers can be trusted.
7. **The frontend container ships a production build now.** Multi-stage Dockerfile: build the SPA, serve it from nginx which proxies `/api` to the backend. Two things follow — Playwright tests the artifact that actually ships rather than a dev server, and Phase 7 inherits a deployable image instead of containerising under deploy pressure. Also means `tsc` runs in CI, which a dev server never forced.

Four further decisions settle the visual design, against a reference set spanning Cursor, Runway, Mistral, Databricks and Zendesk:

8. **Both themes, with a toggle** — dark by default. This *reverses* the draft spec, which listed a dark-mode toggle as out of scope, and the cost is real: every token is defined twice and every screen is worth a look in both. It is accepted because the discipline it forces is worth more than the toggle itself — no component may name a colour, only a token, which is the rule that keeps a hand-written stylesheet from sprawling across ten screens. Dark leads because status badges carry far more signal against a dark panel, and because the demo is a recording.
9. **Developer-tool precision, not editorial.** The Cursor/Linear register: near-monochrome, one restrained accent, hairline borders instead of shadows, tight rhythm, technical values always monospace. Runway and Mistral are beautiful and were rejected for this purpose — their generous editorial spacing fights a screen showing a hundred audit rows, and more scrolling is worse in a demo. Databricks/Zendesk density was taken; their chrome was not.
10. **Self-hosted Inter + JetBrains Mono**, two `woff2` files under `frontend/public/fonts`. Static assets rather than packages, so the zero-dependency rule is respected in substance: nothing is resolved from a registry at runtime and no CDN is contacted — which the container could not do anyway. A real monospace is the single largest legibility win on the audit and evidence screens, and tabular figures stop metric digits jittering while a screen polls.
11. **Left sidebar, compact density.** The sidebar renders only the routes the active role can use, which turns G6.2's role slicing into something visible in one screenshot per persona rather than something asserted in a test nobody looks at.

Gates bind to `data-testid` attributes registered in `frontend/src/testids.ts`, never to copy or CSS classes. Changing a testid is therefore a spec change. The 17-task plan lives in `specs/07-phase6-dashboard.md`; tasks 4 and 17 are Codex's.

**Carried forward unchanged:** the SPA keeps its `sessionStorage` token. httpOnly-cookie + backend session is already recorded in ARCHITECTURE as the hardening step, and it is a backend change rather than a screens change.
...
- ~~**Build Phase 5**~~ on `feat/phase5-eval-observability`: tasks 1, 3–15 (Claude), 2 and 16 (Codex). **All 16 done** — task 16 landed as `reviews/phase5-adversarial-diff-review.md` plus eight probes; seven findings fixed, the eighth recorded as open above (D20). Merged as PR #11, all four CI jobs green on `b837e6f`.
- ~~**Phase 6 spec review**~~ **Resolved** as D21 above.
- ~~**Build Phase 6**~~ on `feat/phase6-dashboard`: tasks 1–3 and 5–16 (Claude), 4 and 17 (Codex). **All 17 done** — task 4 landed as 13 Playwright gates covering G6.1–G6.5; task 17 as `reviews/phase6-adversarial-diff-review.md` plus seven browser probes, all seven findings fixed.

---

## D22 — Phase 6 adversarial round (2026-08-19)

Seven findings, each with a failing browser probe. All seven were real and are fixed. The pattern from Phases 3–5 held once more: **not one was on the happy path.** Every one lived in a role other than the one demoed, a session that had expired, a value nobody would type by accident, or a contract line nobody re-read.

1. **A 401 cleared storage but left the authenticated shell mounted.** The API client dropped the token; the live React token was separate state and the identity query stays fresh for five minutes, so the sidebar and the previous screen's cached data stayed up around an error with no credential left to recover with. Every 401 now announces itself once and the app ends the session in one place, clearing the query cache with it — data fetched under a session that has ended must not outlive it on screen.
2. **The approval card did not say which ticket it was about.** The payload carried `ticket_id` and the card rendered the run, confidence and agent version — so an approver was asked to authorise a priority or team write without being told what would change. The personas doc lists the affected ticket among the card's required contents; it was simply missing.
3. **The edit form accepted any string that differed from the proposal.** "Validated form" was a diff check: `P99-not-a-priority` reached the decision endpoint. Enum fields now validate against the same taxonomy the agent's own output is checked against, offered through a datalist so the field stays typable and therefore still checkable.
4. **Operators and approvers saw no audit trail on run detail.** `GET /api/runs/{id}` already returns tenant-scoped `audit_entries` to every persona; the frontend ignored them, called the administrator-only `/api/audit`, and hid the panel behind `isAdmin`. Fixing it *removed* a call rather than adding an endpoint.
5. **The window selector governed the cards but not the charts.** The donut counted every run in the unwindowed history, so switching to 7 days moved the metrics and left the distribution counting month-old runs beside them — and the activity chart capped every selection at 30 days, silently plotting 30 while labelled 90. Both now derive from one windowed set.
6. **Tickets omitted the required department filter**, and **7. Audit omitted the required date range.** Both are named in the screen contract; both hooks already accepted the parameters. Two contract lines nobody re-read.

**The selector contract has a blind spot worth recording.** Three of these omissions — the department filter, the date range, the affected ticket — were invisible to the gates because `testids.ts` had no ids for things that did not exist. A registry describes what was built, so it cannot ask why something is absent. Codex's probes reached for accessible names and enclosing components instead, which is the right escape hatch; additive ids (D21) closed the gap afterwards. Gates bound to a registry catch regressions in what exists and are structurally blind to omissions.

**One conflict between two Codex artifacts**, resolved toward the newer: the task-4 gate typed free text into whichever control it found first, which after finding 3 is a taxonomy-constrained field that correctly refuses it. The gate now reads allowed values from the DOM — exactly as it already did for `<select>` — rather than assuming every field is free text.
- **The Phase 6 gates arrived red, and each failure was real** — which is the argument for the selector contract (D21). Codex wrote against `frontend/src/testids.ts` without reading the screens, so where the implementation disagreed with the registry the registry won. The sharpest: `citation-<id>` was on every *retrieved* chunk rather than on the ones the model *cited*, so G6.3's question — "does every rendered citation resolve to stored evidence?" — was trivially true and the gate could never have failed. A gate that cannot fail is the failure mode the whole exercise exists to avoid, and only an author who had not seen the implementation would have caught it. The other two were a stale-cache leak (the command palette fetching tickets on every page load under a comment claiming it did not, sharing a cache key with the Tickets screen) and an approval that did not invalidate the ticket it had just rewritten.
- **Six defects in Phase 6 that only a screenshot could find**, each fixed in its own commit: the dashboard bar chart drew nothing (`width` on an inline span); panels overflowed their grid column (`min-width: auto` on grid items); banners broke sentences into columns (flex container around prose); "dark by default" deferred to `prefers-color-scheme`, so a demo recorded on a light-mode machine came out light and browser gates inherited the runner's colour scheme; an em-dash at 30px read as a loading skeleton on every absent metric; and the run screen showed "2 citations" beside "1 cited" because one count was raw and the other distinct. Typechecking and unit tests passed throughout — the lesson is that a UI phase needs a *looking* step the way a backend phase needs a running one.
- **User and role management has no home in the MVP** (D21 decision 5). CLAUDE.md's Administrator persona says "manage users and roles (minimally)", the ten-screen list omits such a screen, and no endpoint exists. Roles are seeded by `scripts/seed.py`. Recorded here so the gap is a decision on the record rather than something a reviewer discovers. Task 2 was written by Claude rather than Codex, like Phase 4's task 13: the gates needed the eval contract that tasks 3 and 5 define, and they landed in the same pass. That is a deviation from the D6 lane and is recorded rather than glossed — a Codex round on `tests/phase5` is still worth having in task 16.
