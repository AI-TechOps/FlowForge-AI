...
6. **No charting dependency.** The dashboard's metrics are a handful of scalars and rates; one hand-rolled SVG bar chart for run outcomes covers the need. The draft spec's "one lightweight chart lib" is superseded — a dependency earning its place on one chart is a dependency that has not earned its place.
