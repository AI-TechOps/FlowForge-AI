...
- **Build Phase 6** on `feat/phase6-dashboard`: tasks 1–3 and 5–16 (Claude), 4 and 17 (Codex). **Tasks 1–3 and 5–16 done**; tasks 4 (the G6.1–G6.5 Playwright gates) and 17 (adversarial pass + cold review) remain Codex's. The CI step that runs them is already wired and reports-and-passes while `tests/phase6` is empty, so the wiring is reviewable before the specs land.
